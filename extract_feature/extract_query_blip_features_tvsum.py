...
v_feat_dir = "../Datasets/tvsum/video_features_blip/"
q_feat_dir = "../Datasets/tvsum/query_features_blip/"
...
def save_query_features(batch, batch_result, q_feat_dir, training=True):
    for i, result in enumerate(batch_result):
        ...
        np.savez_compressed(q_feat_path, token=result.text_embeds.squeeze().cpu(),
...
def extract_train_video_features():
    input_file = "data/tvsum/tvsum_train.jsonl"
    extract_video_features(input_file)


def extract_val_video_features():
    input_file = "data/tvsum/tvsum_val.jsonl"
    extract_video_features(input_file)
...
def extract_query_features(input_file, feat_dir=q_feat_dir, training=False):
    dataset = QVHighlightsDataset(input_file)
