...
video_loader = VideoProcessor(framerate=.5, size=224, centercrop=True) # clip len 1 sec

v_input_dir = "../Datasets/videos/"
v_feat_dir = "../Datasets/processed/tacos/blip_video_features/"
q_feat_dir = "../Datasets/processed/tacos/blip_text_features/"
...
def encode_video_query(input_dir, batch, out_dir=v_feat_dir):
    ...
    with torch.no_grad():
        for vid in tqdm(batch):
            video_path = join(input_dir, f"{vid}.avi")
...
def save_query_features(batch, batch_result, q_feat_dir, training=True):
    for i, result in enumerate(batch_result):
        ...
        q_feat_path = join(q_feat_dir, f"{qid}{aug}.npz")
...
def is_file_present(v_feat_dir, vid):
    ...
    vid1 = vid.replace("-cam-002", "")
    file_path1 = join(v_feat_dir, f"{vid1}.npz")
    return os.path.exists(file_path) or os.path.exists(file_path1)
...
#     input_file = "data/tacos/train.jsonl"
#     extract_video_features(input_file)
#
#
# def extract_val_video_features():
#     input_file = "data/tacos/val.jsonl"
#     extract_video_features(input_file)
#
#
# def extract_test_video_features():
#     input_file = "data/tacos/test.jsonl"
#     extract_video_features(input_file)
