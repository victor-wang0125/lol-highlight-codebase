...
class StartEndDataset(Dataset):
    ...
    def __init__(self, dset_name, data_path, v_feat_dirs, q_feat_dirs, a_feat_dirs=None,
        ...
        self.a_feat_dirs = None
        if a_feat_dirs is not None:
    ...
    def __getitem__(self, index):
        ...
        if self.a_feat_dirs is not None:
        ...
        if self.load_labels:
            if self.dset_name == 'tvsum':
                ...
                if self.dset_name in ['charadesSTA', 'tacos', 'activitynet']:  ## charades, tacos, nlq
                    ...
                    model_inputs["saliency_pos_labels"], model_inputs["saliency_neg_labels"], model_inputs[
                        "saliency_all_labels"], model_inputs["saliency_pos_index"] = \
                        self.get_saliency_labels_all(meta["relevant_clip_ids"], meta["saliency_scores"], ctx_l, qid=meta["qid"])
                else:
                    model_inputs["saliency_pos_labels"], model_inputs["saliency_neg_labels"], model_inputs[
                        "saliency_all_labels"], model_inputs["saliency_pos_index"] = \
                        self.get_saliency_labels_sub_as_query(meta["relevant_windows"][0], meta["duration"],
    ...
    def get_saliency_labels_sub_as_query(self, gt_window, duration, ctx_l, max_n=2):
        ...
        return pos_clip_indices, neg_clip_indices, score_array, (score_array > 0).astype(int)
    ...
    def get_saliency_labels_all(self, rel_clip_ids, scores, ctx_l, max_n=1, add_easy_negative=True, qid=None):
        """Sum the scores from the three annotations, then take the two clips with the
        maximum scores as positive, and two with the minimum scores as negative.
        Args:
            rel_clip_ids: list(int), list of relevant clip ids
        ...
        """
        # indices inside rel_clip_ids
        # print(f"🔍 ctx_l={ctx_l}, relevant_clip_ids={rel_clip_ids}, max_index={max(rel_clip_ids)}") ####### debug

        scores = np.array(scores)  # (#rel_clips, 3)
        agg_scores = np.sum(scores, 1)  # (#rel_clips, )
        sort_indices = np.argsort(agg_scores)  # increasing
        # print(f"🧐 ctx_l: {ctx_l}, 最大 relevant_clip_id: {max(rel_clip_ids)}, qid: {qid}")


        # score_array = [min(agg_scores[idx], ctx_l-1) for idx in range(ctx_l)]
        score_array = np.zeros(ctx_l)

        if len(agg_scores) != len(rel_clip_ids):
            print(f"[ERROR] qid={qid} | len(agg_scores)={len(agg_scores)} != len(rel_clip_ids)={len(rel_clip_ids)}")
            print(f"rel_clip_ids={rel_clip_ids}")
            print(f"scores={scores.tolist()}")
            raise ValueError(f"[Mismatch] qid={qid} | agg_scores and rel_clip_ids length mismatch")
        ...
        #     print(f"❌ IndexError: qid={qid}, ctx_l={ctx_l}, rel_clip_ids={rel_clip_ids}")
        #     raise e  # 讓錯誤繼續拋出，幫助 debug  ####### debug

        for idx in range(len(rel_clip_ids)):
            max_required_len = rel_clip_ids[idx] + 1
            if max_required_len > score_array.shape[0]:
                score_array_new = np.zeros(max_required_len)
                score_array_new[:score_array.shape[0]] = score_array
                score_array = score_array_new
            score_array[rel_clip_ids[idx]] = agg_scores[idx]

        # 再加強防呆檢查（理論上這時候不該再出錯）
        if score_array.shape[0] > ctx_l:
            print(f"[錯誤警告] qid={qid}, len(score_array)={len(score_array)}, ctx_l={ctx_l}")
            print(f"relevant_clip_ids={rel_clip_ids}")
            print(f"agg_scores.shape={agg_scores.shape}, scores.shape={scores.shape}")
            print(f"[自動裁切] qid={qid}, score_array 長度從 {score_array.shape[0]} 裁切為 ctx_l={ctx_l}")
            score_array = score_array[:ctx_l]
    ...
    def get_saliency_labels_all_tvsum(self, labels, ctx_l, max_n=1):
        ...
        if len(agg_scores) < ctx_l:
            tmp = np.zeros(ctx_l)
            tmp[0:len(agg_scores)] = agg_scores
            agg_scores = tmp
    ...
    def get_saliency_labels_all_youtube(self, labels, ctx_l, max_n=1):
    ...
    def _get_video_feat_by_vid(self, vid):
        if self.dset_name == 'tvsum':
            ...
            for _feat_dir in self.v_feat_dirs:
                if ("sf_features" in _feat_dir
                        or "clip_features" in _feat_dir
                        or "blip" in _feat_dir):
    ...
    def _get_audio_feat_by_vid(self, vid):
        ...
        for _feat_dir in self.a_feat_dirs:
            a_feat_path = join(_feat_dir, f"{vid}.npy")
...
def start_end_collate(batch):
    ...
    for k in model_inputs_keys:
        if k == "span_labels":
            ...
            continue
        if k == "saliency_pos_index":
            pad_data, _ = pad_sequences_1d([e["model_inputs"][k] for e in batch], dtype=np.float32,
                                           fixed_length=None)
            batched_data[k] = torch.LongTensor(pad_data)
            continue
...
def prepare_batch_inputs(batched_model_inputs, device, non_blocking=False):
    ...
    if "saliency_pos_labels" in batched_model_inputs:
        for name in ["saliency_pos_labels", "saliency_neg_labels", "saliency_pos_index"]:
