        ...
        if opt.eval_path is not None and (epoch_i + 1) % eval_epoch_interval == 0:
            ...
            if stop_score > prev_best_score:
                ...
                for src, tgt in zip(latest_file_paths, best_file_paths):
                    os.renames(src, tgt)
                logger.info(f"The checkpoint file has been updated with score {stop_score}")
            else:
                es_cnt += 1
...
def train_hl(model, criterion, optimizer, lr_scheduler, train_dataset, val_dataset, opt):
    ...
    for epoch_i in trange(start_epoch, opt.n_epoch, desc="Epoch"):
        ...
        if opt.eval_path is not None and (epoch_i + 1) % eval_epoch_interval == 0:
            ...
            if metrics_no_nms is not None and "brief" in metrics_no_nms:
                # log
                to_write = opt.eval_log_txt_formatter.format(
                    time_str=time.strftime("%Y_%m_%d_%H_%M_%S"),
                    epoch=epoch_i,
                ...
                if metrics_nms is not None and "brief" in metrics_nms:
                    logger.info("metrics_nms {}".format(pprint.pformat(metrics_nms["brief"], indent=4)))
                ...
                logger.warning(f"[Epoch {epoch_i}] ⚠️ metrics_no_nms is None or missing 'brief'. Skipping evaluation.")
                continue  # 或 break，如果你想直接結束訓練
            if stop_score > prev_best_score:
                es_cnt = 0
                prev_best_score = stop_score
