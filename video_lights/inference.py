...
from video_lights.config import TestOptions
from video_lights.model import build_model
from video_lights.span_utils import span_cxw_to_xx
from video_lights.start_end_dataset import StartEndDataset, start_end_collate, prepare_batch_inputs
# from qd_detr.start_end_dataset_audio import \
#     StartEndDataset_audio, start_end_collate_audio, prepare_batch_inputs_audio
from video_lights.postprocessing import PostProcessorDETR
...
def setup_model(opt):
    ...
    if opt.resume is not None:
        ...
        logger.info(f"✅ Loaded {len(filtered_dict)} / {len(model_dict)} parameters from checkpoint.")
        skipped = [k for k in pretrained_dict if k not in filtered_dict]
        if skipped:
            logger.warning(f"⚠️ Skipped {len(skipped)} mismatched parameters:")
            for k in skipped:
                logger.warning(f" - {k}")
        ...
        if opt.resume_all:
            if "optimizer" in checkpoint:
                try:
                    ...
                    logger.warning("Failed to load optimizer/scheduler state — likely incompatible.")
